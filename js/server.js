...
app.get('/api/search', async (req, res) => {
    ...
    const searchPromise = (async () => {
        try {
            ...
            // Try enhanced geocoding service first (MapTiler only)
            try {
                ...
                console.warn(`Enhanced geocoding failed for: ${query}, trying fallback:`, enhancedError.message);

                // Fallback to original geocoding service ONLY if enhanced geocoder completely fails
                results = await geocodingService.searchLocation(query);
                console.log(`Fallback geocoding found ${results.length} results for: ${query}`);
            }
...
app.get('/api/disaster-kit', async (req, res) => {
    try {
        ...
        const analysis = disasterKitService.analyzeLocation(data, locationName);
        res.json(analysis);
