...
const Modal = ({ type, location, onClose }) => {
    ...
    try {
      ...
      switch (type) {
        ...
        case 'ai':
          ...
          break;
        case 'landslide':
          data = await loadLandslideData(lat, lon);
          break;
        default:
          data = null;
      }
  ...
  const loadWeatherData = async (lat, lon) => {
    ...
    const data = await response.json();
    return data.success ? { forecasts: data.forecasts, hourly: data.hourly } : null;
  };
  ...
  const loadLandslideData = async (lat, lon) => {
    const response = await fetch(`/api/predict-landslide?lat=${lat}&lon=${lon}`);
    const data = await response.json();
    return data.success ? data : null;
  };
  ...
  const getModalTitle = () => {
    switch (type) {
      ...
      case 'landslide': return 'Landslide Risk Analysis';
  ...
  const renderModalContent = () => {
    ...
    switch (type) {
      ...
      case 'landslide':
        return renderLandslideContent(modalData);
      default:
  ...
  const renderWeatherContent = (weatherData) => {
    ...
    const avgTemp = forecasts.reduce((sum, f) => sum + f.averageTemperature, 0) / (forecasts.length || 1);
    const avgHumidity = forecasts.reduce((sum, f) => sum + f.averageHumidity, 0) / (forecasts.length || 1);
      ...
      <>
        ...
        {hourly && hourly.length > 0 && (
          <div style={{ marginTop: '20px' }}>
            <h3 style={{ marginBottom: '15px', color: 'var(--primary)' }}>24-Hour Forecast</h3>
            <div className="hourly-forecast-container" style={{
              display: 'flex',
              overflowX: 'auto',
              gap: '12px',
              paddingBottom: '10px',
              ...
              {hourly.slice(0, 24).map((h, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    <div style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.7)' }}>{displayHour}</div>
                    <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>{Math.round(h.temperature)}°</div>
                    <div style={{ fontSize: '0.75rem', color: '#4fc3f7', display: 'flex', alignItems: 'center', gap: '3px' }}>
                      <i className="fas fa-tint"></i> {h.precipitationProbability}%
                    </div>
                  </div>
                );
              })}
  ...
  const renderLandslideContent = (aiData) => {
    if (!aiData) return <div>No Landslide Analysis available</div>;

    const { verdict, message, details } = aiData;

    const getVerdictColor = (v) => {
      if (v === 'DANGER') return '#ff4444';
      if (v === 'CAUTION') return '#ff8800';
      return '#00cc44';
    };
    ...
    return (
      <div className="ai-analysis-content" style={{ padding: '10px' }}>
        ...
        <div className="data-grid" style={{ marginBottom: '30px' }}>
          <div className="data-item">
            <div className="data-value">{details?.features?.soilMoisture?.toFixed(1) || '--'} %</div>
            <div className="data-label">Soil Moisture</div>
          </div>
          <div className="data-item">
            <div className="data-value">{details?.features?.temp?.toFixed(1) || '--'} °C</div>
            <div className="data-label">Temperature</div>
          </div>
          <div className="data-item">
            <div className="data-value">{details?.features?.humidity?.toFixed(1) || '--'} %</div>
            <div className="data-label">Humidity</div>
          </div>
          <div className="data-item">
            <div className="data-value">{details?.features?.precip?.toFixed(1) || '--'} mm</div>
            <div className="data-label">Precipitation</div>
          </div>
          <div className="data-item">
            <div className="data-value">{details?.features?.elevation?.toFixed(1) || '--'} m</div>
        ...
        <div style={{
          ...
          <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: '0.95rem', lineHeight: '1.5' }}>
            {verdict === 'DANGER' ? (
              <>
                <p style={{ marginBottom: '10px' }}><strong>Immediate Action Required:</strong> Terrain indicates high vulnerability to landslides.</p>
                <ul style={{ paddingLeft: '20px', margin: 0 }}>
                  <li>Heavy soil saturation and steep elevations present extreme danger.</li>
                  <li>Evacuate vulnerability zones immediately.</li>
                </ul>
              </>
            ) : verdict === 'CAUTION' ? (
              <>
                <p style={{ marginBottom: '10px' }}><strong>Situational Awareness:</strong> Conditions show moderate risk.</p>
                <ul style={{ paddingLeft: '20px', margin: 0 }}>
                  <li>Be alert while traversing hilly areas.</li>
                  <li>Watch for falling rocks and saturated grounds.</li>
                </ul>
              </>
            ) : (
              <>
                <p style={{ marginBottom: '10px' }}><strong>Normal Conditions:</strong> No severe landslide threats detected.</p>
                <ul style={{ paddingLeft: '20px', margin: 0 }}>
                  <li>Environmental indicators are generally stable.</li>
                </ul>
              </>
            )}
          </div>
