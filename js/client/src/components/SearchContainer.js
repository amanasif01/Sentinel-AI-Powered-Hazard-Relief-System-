...
// Custom Marker Icon to fix React-Leaflet missing icon bug
...
const SearchContainer = ({ onLocationSelect }) => {
  ...
  const MapEvents = () => {
    useMapEvents({
      click(e) {
        ...
        setMapPickerLocation({ lat, lon });
        fetchAddressForPicker(lat, lon);
      },
    });
    return null;
  };

  const fetchAddressForPicker = async (lat, lon) => {
    ...
    try {
      ...
      if (data.success && data.address) {
        setPickerAddress(data.address);
      } else {
        setPickerAddress(`Pinned Location (${lat.toFixed(4)}, ${lon.toFixed(4)})`);
      }
    } catch (error) {
  ...
  const confirmMapSelection = () => {
    if (mapPickerLocation) {
      const location = {
        name: pickerAddress || `Pinned Location (${mapPickerLocation.lat.toFixed(4)}, ${mapPickerLocation.lon.toFixed(4)})`,
        lat: mapPickerLocation.lat,
        lon: mapPickerLocation.lon,
        isCustomPin: true
      };
  ...
  const [isEditingConfirmPin, setIsEditingConfirmPin] = useState(false);
  const [isFetchingConfirmAddress, setIsFetchingConfirmAddress] = useState(false);

  const ConfirmMapEvents = () => {
    useMapEvents({
      click(e) {
        if (!isEditingConfirmPin) return;
        const lat = e.latlng.lat;
        const lon = e.latlng.lng;
        setPendingLocation(prev => ({ ...prev, lat, lon }));
        fetchAddressForConfirm(lat, lon);
      },
    });
    return null;
  };

  const fetchAddressForConfirm = async (lat, lon) => {
    setIsFetchingConfirmAddress(true);
    setPendingLocation(prev => ({ ...prev, name: 'Fetching address...' }));
    try {
      const response = await fetch(`/api/geocode/reverse?lat=${lat}&lon=${lon}`);
      const data = await response.json();
      if (data.success && data.address) {
        setPendingLocation(prev => ({ ...prev, name: data.address, isCustomPin: true }));
      } else {
        setPendingLocation(prev => ({ ...prev, name: `Pinned Location (${lat.toFixed(4)}, ${lon.toFixed(4)})`, isCustomPin: true }));
      }
    } catch (error) {
      console.error('Reverse geocoding error:', error);
      setPendingLocation(prev => ({ ...prev, name: `Pinned Location (${lat.toFixed(4)}, ${lon.toFixed(4)})`, isCustomPin: true }));
    } finally {
      setIsFetchingConfirmAddress(false);
    }
  };
  ...
  return (
    <div className="search-container-new">
      <div className="search-input-wrapper-new" ref={searchInputRef}>
        ...
        {pendingLocation && (
          <div className="location-confirmation-panel" style={{
            ...
            <div style={{ height: '200px', width: '100%', position: 'relative' }}>
              <MapContainer
                key={`${pendingLocation.lat}-${pendingLocation.lon}-${isEditingConfirmPin}`}
                center={[pendingLocation.lat, pendingLocation.lon]}
                zoom={14}
                style={{ height: '100%', width: '100%' }}
                dragging={isEditingConfirmPin}
                touchZoom={isEditingConfirmPin}
                scrollWheelZoom={isEditingConfirmPin}
                doubleClickZoom={isEditingConfirmPin}
              >
                <TileLayer
                ...
                />
                <ConfirmMapEvents />
                <Marker position={[pendingLocation.lat, pendingLocation.lon]} icon={customMarkerIcon} />
              </MapContainer>
              {isEditingConfirmPin && (
                <div style={{
                  position: 'absolute', top: '10px', left: '50%', transform: 'translateX(-50%)',
                  background: 'rgba(0,0,0,0.7)', color: 'white', padding: '5px 15px',
                  borderRadius: '20px', zIndex: 1000, fontSize: '0.8rem', pointerEvents: 'none'
                }}>
            ...
            <div style={{ padding: '15px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button
                  onClick={() => setIsEditingConfirmPin(!isEditingConfirmPin)}
                  style={{
                    padding: '8px 16px',
                    background: isEditingConfirmPin ? 'rgba(76, 175, 80, 0.2)' : 'rgba(255, 255, 255, 0.1)',
                    border: isEditingConfirmPin ? '1px solid #4caf50' : 'none',
                    borderRadius: '6px',
                    color: isEditingConfirmPin ? '#4caf50' : 'white',
                    cursor: 'pointer',
                    transition: 'all 0.3s'
                  }}
                  title="Make the map interactive to adjust the pin"
                >
                  <i className="fas fa-edit" style={{ marginRight: '5px' }}></i>
                  {isEditingConfirmPin ? 'Done Edit' : 'Adjust Pin'}
                </button>
                <button
        ...
        {/* Map Picker Modal */}
        {isMapPickerOpen && (
          <div className="map-picker-modal" onClick={() => setIsMapPickerOpen(false)}>
            <div className="map-picker-content" onClick={e => e.stopPropagation()}>
              <div className="map-picker-header">
                <h3>Pin a Location</h3>
                <button className="close-map-btn" onClick={() => setIsMapPickerOpen(false)}>
              ...
              <div className="map-picker-body">
                ...
                >
                  ...
                  <MapEvents />
                  {mapPickerLocation && (
                    <Marker position={[mapPickerLocation.lat, mapPickerLocation.lon]} icon={customMarkerIcon} />
                  )}
                </MapContainer>
              </div>

              <div className="map-picker-footer">
                <div className="map-picker-location-info">
                  {mapPickerLocation ? (
                    <>
                      <div><b>Lat:</b> {mapPickerLocation.lat.toFixed(4)}, <b>Lon:</b> {mapPickerLocation.lon.toFixed(4)}</div>
                      <div className={`fetching-address ${isFetchingAddress ? 'loading' : ''}`}>
                        <i className={`fas ${isFetchingAddress ? 'fa-spinner fa-spin' : 'fa-map-pin'}`} style={{ marginRight: '5px' }}></i>
                        {pickerAddress}
                      </div>
                    </>
                  ) : (
                    <div>Click anywhere on the map to place a pin</div>
                  )}
                </div>
                <button
                  className="confirm-location-btn"
                  onClick={confirmMapSelection}
                  disabled={!mapPickerLocation || isFetchingAddress}
                >
                  Confirm Location
                </button>
              </div>
