...
const DashboardWithProfileCards = ({ data, onModalOpen, onAuthModalOpen }) => {
  ...
  const getLandslideValue = () => {
    if (!data.landslidePrediction || !data.landslidePrediction.success) return 'Pending...';
    return data.landslidePrediction.verdict || 'Scanning';
  };

  const getLandslideRiskColor = () => {
    if (!data.landslidePrediction || !data.landslidePrediction.success) return '#aa00ff';
    const v = data.landslidePrediction.verdict;
    if (v === 'DANGER') return '#ff0000';
    if (v === 'CAUTION') return '#ff8800';
    return '#00cc44';
  };

  const getLandslideStatsString = () => {
    if (!data.landslidePrediction || !data.landslidePrediction.success || !data.landslidePrediction.details) return 'Analyzing...';
    const features = data.landslidePrediction.details.features;
    if (!features) return 'Analyzing...';
    const sm = features.soilMoisture !== undefined ? `${features.soilMoisture.toFixed(0)}%` : '--';
    const el = features.elevation !== undefined ? `${features.elevation.toFixed(0)}m` : '--';
    return `Soil Moisture: ${sm} • Elev: ${el}`;
  };
  ...
  return (
    <div className="dashboard-with-profile-cards">
      ...
      <div className="profile-cards-grid">
        ...
        {/* Landslide Risk Card */}
        <div className="profile-card-container" onClick={() => handleCardClick('landslide')}>
          <ProfileCard
            name="LANDSLIDE RISK"
            title={`${getLandslideValue()} • ${getLandslideStatsString()}`}
            handle="landslide-insights"
            status={data.landslidePrediction ? "Analyzed" : "Processing"}
            contactText="View Safety Report"
            avatarUrl="/landslide_premium.png"
            showUserInfo={false}
            enableTilt={true}
            enableMobileTilt={false}
            behindGradient={`radial-gradient(farthest-side circle at var(--pointer-x) var(--pointer-y),hsla(0,0%,100%,0.2) 4%,hsla(0,0%,100%,0.1) 10%,transparent 50%), radial-gradient(circle at 50% 50%, ${getLandslideRiskColor()} 0%, transparent 70%)`}
            innerGradient="linear-gradient(145deg,rgba(255,255,255,0.1) 0%,rgba(0,0,0,0.1) 100%)"
            onContactClick={() => handleCardClick('landslide')}
          />
        </div>
      </div>
