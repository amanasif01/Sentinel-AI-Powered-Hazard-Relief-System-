...
const RiskAssessmentScreen = ({ selectedLocation, onLocationSelect, onModalOpen, onAuthModalOpen }) => {
  ...
  const loadLocationData = async (lat, lon) => {
    ...
    const [rainfallData, waterbodyData, weatherData, terrainData, predictionData, landslideData] = await Promise.allSettled([
      ...
      fetch(`/api/predict-landslide?lat=${lat}&lon=${lon}`).then(res => res.json())
    ]);
    ...
    setDashboardData({
      ...
      landslidePrediction: (landslideData && landslideData.status === 'fulfilled') ? landslideData.value : null
    });
  };
