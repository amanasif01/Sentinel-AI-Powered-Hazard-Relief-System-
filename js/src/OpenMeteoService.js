...
class OpenMeteoService {
    ...
    async getCurrentWeather(latitude, longitude) {
        try {
            // Fetch current temperature and rain
            const url = `${this.BASE_URL}?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,rain,weather_code&timezone=auto`;

            const response = await this.makeHttpRequest(url);
            const data = JSON.parse(response);

            if (!data.current) {
                return { rain: 0, temp: 0 };
            }
