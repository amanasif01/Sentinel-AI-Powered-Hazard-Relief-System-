...
class GeocodingService {
    ...
    createCleanDisplayName(city, state, country, originalDisplayName, lat, lon) {
        // preserve the original name's specific location info (usually the first part)
        let specificFeature = "";
        if (originalDisplayName) {
            const parts = originalDisplayName.split(',').map(p => p.trim());
            // If the first part differs from city/state/country, it's a specific place
            if (parts.length > 0 &&
                parts[0] !== city &&
                parts[0] !== state &&
                parts[0] !== country) {
                specificFeature = parts[0];
            }
        }

        let nameParts = [];
        if (specificFeature) nameParts.push(specificFeature);
        if (city && city !== '') nameParts.push(city);
        if (state && state !== '') nameParts.push(state);
        if (country && country !== '') nameParts.push(country);

        if (nameParts.length > 0) {
            // Deduplicate consecutive identical parts just in case
            return nameParts.filter((item, pos, arr) => pos === 0 || item !== arr[pos - 1]).join(", ");
        } else {
    ...
    prioritizePakistanResults(results, query) {
        ...
        for (const result of results) {
            ...
            // Enhanced matching utilizing fuzzysort for typo tolerance
            let matchScore = 0;
            const fuzzysort = require('fuzzysort');

            // Check for fuzzy matching of city name
            const fuzzyCityMatch = fuzzysort.single(queryLower, city);

            // Highest priority for exact city match
            if (city === queryLower) {
                matchScore = 150;
            } else if (fuzzyCityMatch && fuzzyCityMatch.score > -15) { // very close typo
                matchScore = 120 + fuzzyCityMatch.score;
            } else if (city.startsWith(queryLower)) {
                matchScore = 80;
            } else if (city.includes(queryLower)) {
                matchScore = 60;
            } else {
                // Check fuzzy matching for full display name
                const fuzzyNameMatch = fuzzysort.single(queryLower, displayName);
                if (fuzzyNameMatch && fuzzyNameMatch.score > -20) {
                    matchScore = 50 + fuzzyNameMatch.score;
                }
